...
import { clearCartActionCreator, decreasePizzasActionCreator, increasePizzasActionCreator, removePizzasActionCreator } from '../../redux/cart-reducer';
import CartPizzaBlock from './CartPizzaBlock/CartPizzaBlock';
import cartEmptyImage from './../../assets/img/empty-cart.png';
import { useHistory } from "react-router-dom";

const Cart = (props) => {
    const { totalPrice, totalPizzas, items } = useSelector(({ cart }) => cart);
    const pizzas = Object.keys(items).map((el) => items[el]);
    ...
    const increasePizza = (id) => {
        dispatch(increasePizzasActionCreator(id));
    };
    ...
    return (<>
        {totalPizzas ? (<div className="container container--cart">
            <div className="cart">
                ...
                <div className="content__items">
                    {pizzas.map((pizza) => {
                        return <CartPizzaBlock key={pizza.items[0].id} id={pizza.items[0].id}
                            name={pizza.items[0].name} size={pizza.items[0].size}
                            type={pizza.items[0].type} totalPrice={pizza.totalPrice}
                            amount={pizza.items.length} removePizza={removePizza}
                            decreasePizza={decreasePizza} increasePizza={increasePizza}
                        />
                    }
                    )}
